...
<section class="grid gap-4">
    <x-ui.alert-messages />
    <x-ui.table.header>
        <x-ui.links.primary href="{{ route('admin.categories.create') }}" title="Ad New Category" />
                    ...
                    <x-ui.table.tr wire:key="{{ $data->id }}">
                        ...
                        <x-ui.table.td>
                            @if ($data->status == 'published')
                                <x-ui.badges.success :content="__('Published')" />
                            @else
                                <x-ui.badges.danger :content="__('Un Published')" />
                            @endif
                        </x-ui.table.td>
