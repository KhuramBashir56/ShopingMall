...
<section class="grid gap-4">
    <x-panel.ui.page-header>
        <x-ui.links.primary href="{{ route('admin.products.units.list') }}" title="Stock Units List" />
    </x-panel.ui.page-header>
    <x-panel.ui.card>
        <div class="grid md:grid-cols-2 gap-x-6 w-full">
            ...
            <x-ui.form.label :title="__('Unit Code')" :for="__('code')">
                <x-ui.form.input type="text" wire:model='code' :for="__('code')" placeholder="Unit code goes here..." maxlength="2" />
                @error('code')
                    <x-ui.form.input-error :message="$message" />
                @enderror
            </x-ui.form.label>
            ...
            <div class="md:col-span-2">
                <x-ui.form.label :title="__('Brand Description')" :for="__('description')">
                    <x-ui.form.textarea wire:model='description' :for="__('description')" placeholder="Brand description goes here..." maxlength="255" />
                    @error('description')
                        <x-ui.form.input-error :message="$message" />
                    @enderror
                </x-ui.form.label>
